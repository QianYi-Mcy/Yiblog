...
async function loadCharts() {
  ...
  initChart(
    visitTrendRef.value,
    lineOption(
      visitRes.data.map((i) => i.date),
      visitRes.data.map((i) => i.count),
      '#1890ff'
    )
  )

  initChart(
    visitorTrendRef.value,
    lineOption(
      visitorRes.data.map((i) => i.date),
      visitorRes.data.map((i) => i.count),
      '#52c41a'
    )
  )
  ...
  initChart(topArticlesRef.value, {
